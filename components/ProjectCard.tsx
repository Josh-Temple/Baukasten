...
export const ProjectCard: React.FC<{ project: Project; onClick: () => void }> = ({ project, onClick }) => {
  ...
  return (
    <article className="grid grid-cols-12 gap-6 items-center group">
      <button onClick={onClick} aria-label={`View ${project.title} details`} className="col-span-5 w-full aspect-[3/5] rounded-[2rem] relative flex items-center justify-center text-white overflow-hidden transition-transform duration-300 hover:-translate-y-1 active:scale-95 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4" style={{ backgroundColor: project.themeColor }}>
      ...
      <div className="col-span-7 min-w-0">
        ...
        <h3 className="text-xl font-black tracking-tight leading-tight text-slate-900 mb-3 break-words">{project.title}</h3>
        <p className="text-[0.8rem] text-slate-600 leading-snug font-bold mb-5">{project.oneLiner}</p>
