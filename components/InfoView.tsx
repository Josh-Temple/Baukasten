...
export const InfoView: React.FC<{ onBack: () => void }> = ({ onBack }) => (
  <div className="min-h-screen bg-[#F0F2F5] text-slate-900 pb-24 overflow-x-hidden">
    ...
    <main className="max-w-md mx-auto px-8 pt-16">
      ...
      <div className="space-y-20">
        <section><h1 className="text-3xl font-black tracking-tighter mb-6">Why Baukasten exists</h1><p className="text-base font-bold leading-relaxed text-slate-600">Baukasten is a collection of small learning and thinking tools built around recurring problems. The projects are shaped by repeated personal use, then tested or refined when they may help other people.</p></section>
        <section><h2 className="text-xl font-black mb-6">How projects are selected</h2><ul className="space-y-3">{['Solves a recurring problem', 'Has evidence of real use', 'Supports a clear user task', 'Has an honest current status', 'Adds a distinct idea to the collection'].map(item => <li key={item} className="flex gap-3 text-sm font-bold text-slate-600"><span className="text-[#BD5B5B]">■</span>{item}</li>)}</ul></section>
        <section><h2 className="text-xl font-black mb-6">Status meanings</h2><dl className="space-y-5">{statuses.map(([term, meaning]) => <div key={term}><dt className="text-xs font-black uppercase tracking-widest text-slate-800">{term}</dt><dd className="mt-1 text-sm leading-relaxed text-slate-600">{meaning}</dd></div>)}</dl></section>
        <section><h2 className="text-xl font-black mb-6">Current approach</h2><p className="text-base font-bold leading-relaxed text-slate-600">Use before expansion. Validate before major features. Keep specialist tools specialist when their narrow focus is their strength.</p></section>
      </div>
