  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] text-slate-900 pb-32 overflow-x-hidden">
      ...
      <main className="max-w-4xl mx-auto px-6 lg:px-8">
        <header className="py-16 lg:py-20 text-center max-w-2xl mx-auto">
        ...
        <article className="space-y-16 lg:space-y-20 border-t border-slate-200 pt-16 lg:pt-20">
          {[["Who it is for", project.audience], ["The problem", project.fullProblem], ["How it helps", project.fullApproach]].map(([title, copy]) => copy && <section key={title} className="max-w-2xl mx-auto"><h2 className="text-[0.65rem] font-black text-slate-500 uppercase tracking-[0.3em] mb-5">{title}</h2><p className="text-lg text-slate-700 leading-relaxed font-bold">{copy}</p></section>)}
          {project.howItWorks?.length ? <section><h2 className="text-[0.65rem] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">How it works</h2><div className="space-y-4 md:grid md:grid-cols-2 md:gap-4 md:space-y-0">{project.howItWorks.map((step, index) => <div key={step.title} className="bg-white border border-slate-200 rounded-2xl p-6"><p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">0{index + 1} · {step.title}</p><p className="text-sm font-bold text-slate-600 leading-relaxed">{step.desc}</p></div>)}</div></section> : null}
          {project.screenshots?.length ? <section><h2 className="text-[0.65rem] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">Screenshots</h2><div className="flex overflow-x-auto snap-x gap-6 pb-5 md:grid md:grid-cols-2 md:overflow-visible md:pb-0">{project.screenshots.map(screen => <figure key={screen.src} className="min-w-[78%] snap-center md:min-w-0"><img src={assetUrl(screen.src)} alt={`${project.title}: ${screen.caption}`} className="w-full rounded-[2rem] border-4 border-white shadow-float" loading="lazy"/><figcaption className="mt-3 text-xs font-bold text-slate-500 text-center">{screen.caption}</figcaption></figure>)}</div></section> : null}
          {(project.techStack?.length || project.repoLink) && <section className="max-w-2xl mx-auto bg-white rounded-[2rem] p-8 border border-slate-200"><h2 className="text-[0.65rem] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">Technical details</h2>{project.techStack?.length ? <div className="flex flex-wrap gap-2">{project.techStack.map(tech => <span key={tech} className="px-3 py-2 bg-[#F0F2F5] rounded-xl text-xs font-bold text-slate-600">{tech}</span>)}</div> : null}{project.repoLink && project.repoLink !== primaryLink ? <a href={project.repoLink} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex font-black text-sm underline underline-offset-4">View source</a> : null}</section>}
        </article>
      </main>
    </div>
  );
